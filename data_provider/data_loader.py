...
class Dataset_SQLiteFolder(Dataset):
    """Load a folder of sqlite3 database files with a timestamp column.

    Parameters
    ----------
    table_name : str
        Name of the table to read from each database file.
    state : {'all', 'running', 'standby'}
        Which rows to keep. Rows are labelled by evaluating ``state_condition``
        if provided, otherwise ``state_column`` or ``speed_column``.
    state_condition : str, optional
    ...
    def __init__(self, args, root_path, flag='train', size=None,
        ...
        if table_name is None:
            raise ValueError('table_name must be provided for sqlitefolder dataset')
        self.args = args
        if size is None:
            self.seq_len = 96
            self.label_len = 48
    ...
    def _load_folder(self):
        files = sorted(glob.glob(os.path.join(self.root_path, '*.db')))
        if not files:
            raise FileNotFoundError(f'No sqlite3 files found in {self.root_path}')
        frames = []
        for f in files:
            ...
            if self.state != 'all':
                if self.state_condition is not None:
                    mask = df.eval(self.state_condition)
                    df = df[mask] if self.state == 'running' else df[~mask]
                elif self.state_column in df.columns:
                    df = df[df[self.state_column] == 1] if self.state == 'running' else df[df[self.state_column] == 0]
                elif self.speed_column in df.columns:
                    if self.state == 'running':
                        df = df[df[self.speed_column] > self.speed_threshold]
                    else:
